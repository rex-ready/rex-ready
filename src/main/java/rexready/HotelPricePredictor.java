...
public class HotelPricePredictor
{
	...
	public float updateDeltaForAuction(int auctionID, int t)
	{
		...
		if (closed[counterpartID] == 0) //if the counterpart auction is still open
		{
			...
			//if p is high and CP is not high and c is not quick, delta is medium
		...
		else if (t - closed[counterpartID] < 6) //if the counterpart auction has just closed (within a minute)
		{
